...
/**
 * WindowManager - a basic window manager that provides stub methods for handling mouse events, etc.
 * This is implemented as an abstract class so that it can be customized for specific uses.
 *
 * @author Brian Kell
 * @version March 2016
 */
public abstract class WindowManager extends JFrame
    ...
    /**
     * WindowManager - Default constructor for this abstract class.
     */    
    public WindowManager() {
        super();           // call the JFrame default constructor (no explicit size)
        setVisible(true);  // then make the window visible

        // make this window notice when components are added to it
        getContentPane().addContainerListener(this);
    }

    /**
     * WindowManager(int width, int height) - Constructor with size specified.
     *
     ...
     */
        ...
        super();
        setSize(width, height);  // this is an addition from the basic constructor
        setVisible(true);
        getContentPane().addContainerListener(this);
    }

    /**
     * WindowManager(String title, int width, int height) - Constructor with size specified.
     *
     * @param title a String to show in the title bar
     * @param width the width of the window
     * @param height the height of the window
     */
        ...
        super();
        setSize(width, height);  // this is an addition from the basic constructor
        setTitle(title);         // this is an addition from the basic constructor
        setVisible(true);
        getContentPane().addContainerListener(this);
    }

    /**
     * componentAdded - required by the ContainerListener interface.
     * 
     * @param event when an event happens to the container
     */
    ...
    /**
     * componentRemoved - required by the ContainerListener interface.
     * 
     * @param event when an event happens to the container
     */
    ...
    //* clicks at this point, the first four can be stubs that don't do anything.
    //* We can expand them to do something in the future as desired.
    //======================================================================
    public void mouseEntered(MouseEvent event) {
    }
    ...
    /**
     * mouseClicked - activated when a MouseClick event is detected.
     * 
     * @param event which widget detected the MouseClick event
     */
    public void mouseClicked(MouseEvent event) {
    ...
    //* two can be stubs that don't do anything.
    //* We can expand them to do something in the future as desired.
    //======================================================================
    public void keyPressed(KeyEvent event) {
    }
    ...
    /**
     * keyTyped - activated when a key press event is detected.
     * 
     * @param event which widget detected the KeyEvent 
     */
    public void keyTyped(KeyEvent event) {
        ...
        if (event.getKeyChar() == '\n') {
            ...
            textEntered((JTextField) component);  // call the correct abstract method
        }
    }
    ...
    //* is lost at this point, the first method can be a stub that doesn't do
    //* anything. We can expand it to do something in the future as desired.
    //======================================================================
    public void focusGained(FocusEvent event) {
    }

    /**
     * focusLost - activated when the focus leaves a widget.
     * 
     * @param event which widget last focus 
     */
    public void focusLost(FocusEvent event) {
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        if (componentType.contains("JComboBox")) {
            selectionMade((JComboBox) component); // call the correct abstract method
        } else if (componentType.contains("JMenuItem")) {
            selectionMade((JMenuItem) component); // call the correct abstract method
        } else if (componentType.contains("JButton")) {
    ...
    public void stateChanged(ChangeEvent event) {
        ...
        if (componentType.contains("JSlider")) {
            sliderChanged((JSlider) component); // call the correct abstract method
        }
    }
    ...
    /**
     * Mouse clicked on a JLabel.
     */
        public void labelClicked(JLabel whichLabel) {
    }

    /**
     * Text entered in a JTextField.
     */
    ...
    /**
     * Focus leaves a JTextField
     */
    public void focusLost(JTextField whichField) {
    }

    /**
     * Focus leaves a JButton
     */
    ...
    /**
     * A selection is made from a "ComboBox" (a drop-down list of choices).
     */
    public void selectionMade(JComboBox whichComboBox) {
    }

    /**
     * A selection is made from a Menu
     */
